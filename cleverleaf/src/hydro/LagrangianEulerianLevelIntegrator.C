    ...
    const std::shared_ptr<hier::BaseGridGeometry>& transfer_geom)
{
  ...
  if((var_exchanges & NO_EXCH) != NO_EXCH) {
    if(var->getName() == "velocity" || var->getName()=="vertexdeltas" || var->getName()=="vertexcoords") {
      refine_operator = transfer_geom->lookupRefineOperator(var, "RAJA_LINEAR_REFINE");
      coarsen_operator = transfer_geom->lookupCoarsenOperator(var, "RAJA_CONSTANT_COARSEN");
    } else if (var->getName() == "massflux" || var->getName() == "volflux") {
      refine_operator = transfer_geom->lookupRefineOperator(var, "RAJA_FIRST_ORDER_REFINE");
    } else if (var->getName().find("RAJA") != std::string::npos) {
    } else if (var->getName().find("tags") != std::string::npos) {
    } else {
      refine_operator = transfer_geom->lookupRefineOperator(var, "RAJA_CONSERVATIVE_LINEAR_REFINE");
    }
  }

  if((var_type & FIELD) == FIELD) {
    SAMRAI::hier::PatchDataRestartManager::getManager()->
      registerPatchDataForRestart(current_id);
    ...
    if (var->getName() == "velocity") {
      ...
      d_coarsen_field_data->registerCoarsen(
          current_id,
          current_id,
          transfer_geom->lookupCoarsenOperator(var, "RAJA_VOLUME_WEIGHTED_COARSEN"));
    } else if (var->getName() == "energy") {
      d_coarsen_field_data->registerCoarsen(
          current_id,
          current_id,
          transfer_geom->lookupCoarsenOperator(var, "RAJA_MASS_WEIGHTED_COARSEN"));
    }
  }
  ...
  if((var_type & INDICATOR) == INDICATOR) {
    d_coarsen_level_indicator->registerCoarsen(
        current_id,
        current_id,
        transfer_geom->lookupCoarsenOperator(var, "RAJA_CONSTANT_INDICATOR_COARSEN"));
  }
  ...
  if((var_exchanges & PRE_SWEEP_1_CELL_EXCH) == PRE_SWEEP_1_CELL_EXCH) {
    if(var->getName() == "volflux" || (var->getName().find("RAJA") != std::string::npos)) {
  ...
  if((var_exchanges & PRE_SWEEP_1_MOM_EXCH) == PRE_SWEEP_1_MOM_EXCH) {
    if(var->getName() == "massflux" || (var->getName().find("RAJA") != std::string::npos)) {
...
void LagrangianEulerianLevelIntegrator::advecCellSweep1(
    const std::shared_ptr<hier::PatchLevel>& level)
{
  const int sweep_number=1;
  const int direction = advect_x ? Coord::X : Coord::Y;
...
void LagrangianEulerianLevelIntegrator::advecCellSweep2(
    const std::shared_ptr<hier::PatchLevel>& level)
{
  const int sweep_number=2;
  const int direction = advect_x ? Coord::Y : Coord::X;
